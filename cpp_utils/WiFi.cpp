 ...
 */
uint8_t WiFi::connectAP(const std::string& ssid, const std::string& password, const std::string& clientName, bool waitForConnection) {
    ...
    int8_t power = 0;
    errRc =:: esp_wifi_get_max_tx_power(&power);
    ESP_LOGI(LOG_TAG, "Sta power = %d", power);
...
/* STATIC */ esp_err_t WiFi::eventHandler(void* ctx, system_event_t* event) {
    ...
    if(event->event_id == SYSTEM_EVENT_STA_GOT_IP || event->event_id == SYSTEM_EVENT_STA_DISCONNECTED) {
        ...
        if (event->event_id == SYSTEM_EVENT_STA_GOT_IP) {
              // If we connected and have an IP, change the status to ESP_OK.  Otherwise, change it to the reason code.
        	tcpip_adapter_ip_info_t info =  pWiFi->getApIpInfo();
            struct in_addr ip_addr;
            ip_addr.s_addr = info.ip.addr;
            ESP_LOGD(LOG_TAG, "IP Address is %s", inet_ntoa(ip_addr));
        ...
        if (!m_clientName.empty())
            ESP_ERROR_CHECK(tcpip_adapter_set_hostname(TCPIP_ADAPTER_IF_STA, m_clientName.c_str()));  
    }
    return rc;
} // eventHandler
...
void WiFi::startAP(const std::string& ssid, const std::string& password, wifi_auth_mode_t auth) {
    startAP(ssid, password, auth, 6, false, 4);
} // startAP
...
void WiFi::startAP(const std::string& ssid, const std::string& password, wifi_auth_mode_t auth, uint8_t channel, bool ssid_hidden, uint8_t max_connection) {
    ...
    int8_t power = 0;
    errRc =::esp_wifi_get_max_tx_power(&power);
    ESP_LOGI(LOG_TAG, "AP power = %d", power);
    ESP_LOGD(LOG_TAG, "<< startAP");
} // startAP

/**
 * @brief add the event handler to the list of event handlers
 * @param[in] wifiEventHandler The class that will be used to process events.
 */
void WiFi::addWifiEventHandler(WiFiEventHandler* wifiEventHandler) {
    ESP_LOGD(LOG_TAG, ">> addWifiEventHandler: 0x%d", (uint32_t)wifiEventHandler);
    // insert it into the linked list.
    wifiEventHandler->setNextHandler(m_pWifiEventHandler);
    this->m_pWifiEventHandler = wifiEventHandler;
    ESP_LOGD(LOG_TAG, "<< addWifiEventHandler");
} // addWifiEventHandler

void WiFi::removeWifiEventHandler(WiFiEventHandler* wifiEventHandler) {
    ESP_LOGD(LOG_TAG, ">> removeWifiEventHandler: 0x%d", (uint32_t)wifiEventHandler);
    // remove it from the linked list.
    WiFiEventHandler **w = & m_pWifiEventHandler;
    while (*w != nullptr)
    {   
        if (*w == wifiEventHandler)
            *w = (*w)->getNextHandler();
    }
    ESP_LOGD(LOG_TAG, "<< removeWifiEventHandler");
...
static wifi_config_t smart_wifi_config;
...
void smartconfig_example_task(void * parm) {
    ...
    while (1) {
        ...
        if (uxBits & ESPTOUCH_DONE_BIT) {
            ...
            WiFiEventHandler* sev = wifi->getSmartEventHandler();
            wifi->removeWifiEventHandler(sev);
            delete sev;
            vTaskDelete(NULL);
        }
    }
...
class SmartConfigEventHandler : public WiFiEventHandler {
public:
    SmartConfigEventHandler(WiFi *pWiFi) {
        m_WiFi = pWiFi;
    }

    esp_err_t staStart() {
        ESP_LOGI(TAG, "staStart");
        m_WiFi->stationStarted();
        return ESP_OK;
    }
        
    esp_err_t staGotIp(system_event_sta_got_ip_t info) {
        ESP_LOGI(TAG, "staGotIP");
        xEventGroupSetBits(wifi_event_group, CONNECTED_BIT);
        return ESP_OK;
    }
    esp_err_t staDisconnected(system_event_sta_disconnected_t info)
    {
        ESP_LOGI(TAG, "staDisconnect");
        esp_wifi_connect();
...
void WiFi::stationStarted()
{
    if (m_doingSmartConfig)
    {
        xTaskCreate(smartconfig_example_task, "smartconfig_example_task", 4096, this, 3, NULL);
        m_doingSmartConfig = false;
    }
}


uint8_t WiFi::doSmartConnect() {
    init();
 //   tcpip_adapter_init();
    wifi_event_group = xEventGroupCreate();
    m_doingSmartConfig = true;
 //   setIPInfo("192.168.4.1", "192.168.4.1", "255.255.255.0");
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    m_smart_event_handler = new SmartConfigEventHandler(this);
    addWifiEventHandler(m_smart_event_handler);
...
wifi_config_t& WiFi::getSmartConfigData()
{
    return smart_wifi_config;
}