...
Socket Socket::accept() {
    ...
    socklen_t sin_size;
    int clientSockFD =:: accept(m_sock, (struct sockaddr *)&client_addr, &sin_size);
...
int Socket::setSocketOption(int option, void* value, size_t len) {
    ...
    //    if (res < 0) {
    //        ESP_LOGE(LOG_TAG, "Error in SetSocketOption %X : %d", option, errno);
    //    }
        return res;
} // setSocketOption
 ...
 * @brief Set the socket option.
 */
int Socket::setMulticast(uint8_t ttl, std::string address) {
    int res =:: setsockopt(m_sock, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl));
    if (res < 0) {
        ESP_LOGE(LOG_TAG, "Error in SetMulticast %X : %d", IP_MULTICAST_TTL, errno);
        return res;
    }
    struct ip_mreq imreq = {{0}, {0}};
    struct in_addr iaddr = {0};
    int err = 0;
    imreq.imr_interface.s_addr = IPADDR_ANY;
    const char *adr = address.c_str();

    // Configure multicast address to listen to
    err = inet_aton(adr, &imreq.imr_multiaddr.s_addr);
    if (err != 1) {
        ESP_LOGE(LOG_TAG, "Configured IPV4 multicast address '%s' is invalid.", adr);
        return err;
    }
    ESP_LOGI(LOG_TAG, "Configured IPV4 Multicast address %s", adr);

    err = setsockopt(m_sock, IPPROTO_IP, IP_MULTICAST_IF, &iaddr, sizeof(struct in_addr));
    if (err < 0) {
        ESP_LOGE(LOG_TAG, "Failed to set IP_MULTICAST_IF. Error %d", errno);
        return err;
    }

    err = setsockopt(m_sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, &imreq, sizeof(struct ip_mreq));
    if (err < 0) {
        ESP_LOGE(LOG_TAG, "Failed to set IP_ADD_MEMBERSHIP. Error %d", errno);
        return err;
    }
    return err;
}
...
//        ESP_LOGE(LOG_TAG, "SetTimeout setsockOpt failed on send");
        return -1;
    }
    if (setSocketOption(SO_SNDTIMEO, (char *)&tv, sizeof(tv)) < 0) {
//        ESP_LOGE(LOG_TAG, "SetTimeout setsockOpt failed on recv");
        return -1;
    }
    return 0;
}
...
/*int Socket::send(xstring value) const {
    ESP_LOGD(LOG_TAG, "send: Binary of length: %d", value.length());
    return send((uint8_t *)value.data(), value.size());
} // send
*/
